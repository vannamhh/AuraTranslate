...
//! vì lớp lỗi mà AD-17 tồn tại để chặn là *"Glossary bắt được một biến thể mà TM ⛔
...
/// cùng một thuật ngữ khớp ở câu này và ⛔ không khớp ở câu kia — đúng lớp lỗi *"⛔
...
/// ⚠️ ⛔ Cũng đừng đi vòng qua `Jieba::tokenize(s, TokenizeMode::Default, hmm)` — nó
...
/// | `mice` | `mice` | `mouse` | `mous` | ⛔ |
/// | `better` | `better` | `good` | `good` | ⛔ |
/// | 🔴 `happiest` | `happiest` | `happy` | `happi` | ⛔ |
///
...
///
/// **`Zh` ⇒ n-gram KÝ TỰ** — cửa sổ trượt theo ký tự, ⛔ **không** theo token và ⛔
...
/// ASCII thuần và sai với mọi thứ khác**, tức một lỗi đi trọn bộ test tiếng Anh mà ⛔
/// không đỏ một ca nào.
///
/// ─────────────────────────────────────────────────────────────────────────────
...
/// | `文化`, thuật ngữ `文` | `文化` *(một token)* | ⛔ **từ chối** — đầu cuối rơi giữa token |
///
