...
//!    lượt chạy cho hai thứ tự — tức một ca **flaky**, và một ca flaky **bị gỡ** chứ không
...
/// `entry_ids` chỗ gọi truyền vào, không cũng không phụ thuộc cách chia lô**. Hai thứ tự khác
...
pub(super) fn read_senses(db: ReadHandle<'_>, entry_ids: &[i64]) -> SqlResult<Vec<SenseRecord>> {
    ...
    let mut senses = run_batched(db, &SENSE_SQL, entry_ids, |row| {
        ...
        Ok(SenseRecord {
            ...
            entry_id: row.get(1)?,
            pos: row.get(2)?,
            // FR35 — vị từ quyết ở RUST, không ở webview (AD-1). Xem `is_foreign_lang`.
    ...
    for (sense_id, example) in run_batched(db, &EXAMPLE_SQL, &sense_ids, |row| {
        ...
        Ok((
            ...
            ExampleRecord {
                ...
                // AC4 — "cùng luật" nghĩa là cùng MỘT hàm, không hai bản chép.
