...
fn normalize_rs_carries_zero_lines_naming_a_sentence_terminator_character() {
    ...
    // Scan product code only: the file's own `#[cfg(test)]` fixtures legitimately need real
    // terminator chars, which is not the table copy this assertion guards against.
    let product_only = without_test_modules(&text);
...
fn the_normalize_functions_have_exactly_eight_named_product_call_sites() {
    ...
    for (rel, text) in &files {
        ...
        // Scan text with `#[cfg(test)]` blocks removed, not the raw file: a test-only call
        // to `normalize::normalize` elsewhere in the tree must not count as a product call
        // site.
        let product_only = without_test_modules(text);
