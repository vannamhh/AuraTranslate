...
fn a_kept_image_gets_a_real_file_on_disk_and_an_asset_row_with_the_right_anchor() {
    ...
    let opened = create_work(&root, "Anh Giua Bai", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()))
...
fn an_image_without_alt_still_keeps_its_position() {
    ...
    let opened = create_work(&root, "Anh Khong Alt", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()))
...
fn an_svg_response_is_rejected_writes_no_file_and_the_chapter_text_survives_intact() {
    ...
    let opened = create_work(&root, "Anh SVG", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &domain_log_state)
...
fn a_failed_image_fetch_does_not_fail_the_whole_import() {
    ...
    let opened = create_work(&root, "Anh Hong", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()))
...
fn the_same_image_url_across_two_chapters_is_fetched_exactly_once_and_shares_one_file() {
    ...
    let opened = create_work(&root, "Anh Trung URL", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()))
        .expect("tao Tac pham that bai");
...
fn a_failing_image_url_reused_across_two_chapters_counts_two_failed_positions_from_exactly_one_network_call() {
    ...
    let opened = create_work(&root, "Anh Hong Trung URL", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()))
        .expect("mot anh mang loi khong duoc lam trot ca luot nhap");
...
fn a_disk_write_failure_mid_asset_write_fails_the_whole_import_and_removes_the_atproj_folder() {
    ...
    let result = create_work(&root, "Dia Day", "en", "", shape, encoding_rs::UTF_8, Vec::new(), None, Vec::new(), &std::sync::Mutex::new(Vec::new()));
