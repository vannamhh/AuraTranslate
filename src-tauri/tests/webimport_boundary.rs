...
mod boundary_scan;
use boundary_scan::{code_lines, src_root, without_test_modules};
...
fn fetcher_carries_zero_lines_naming_a_content_parsing_crate() {
    ...
    let offenders: Vec<String> = code_lines(&without_test_modules(&text))
...
fn reqwest_is_named_only_inside_core_webimport_or_core_ai() {
    ...
    for (rel, text) in &files {
        for (line, code) in code_lines(&without_test_modules(text)) {
...
fn extractor_carries_at_least_one_line_of_real_code_that_reparses_the_original_document() {
    let text = text_of("core/webimport/extractor.rs");
    let hits = code_lines(&without_test_modules(&text))
...
fn the_reparse_check_would_actually_flag_a_gutted_body_that_only_mentions_it_in_a_comment() {
    ...
    let hits = code_lines(&without_test_modules(gutted))
        .filter(|(_, code)| line_parses_the_original_document_a_second_time(code))
        .count();
    assert_eq!(
        hits, 0,
    ...
    let hits_real = code_lines(&without_test_modules(real))
        .filter(|(_, code)| line_parses_the_original_document_a_second_time(code))
        .count();
...
// ═════════════════════════════════════════════════════════════════════════════════
// AD-40-specific self-check; generic `without_test_modules` cases live in
// `boundary_scan_contract.rs`.
// ═════════════════════════════════════════════════════════════════════════════════
...
fn a_forbidden_token_seeded_only_inside_a_cfg_test_block_is_not_counted_against_the_product_code() {
    ...
    let offenders: Vec<String> = code_lines(&without_test_modules(seeded))
        .filter(|(_, code)| line_names_any_forbidden_token(code, &CONTENT_PARSING_TOKENS))
        .map(|(_, code)| code)
        .collect();
    assert!(
    ...
    assert_eq!(
        code_lines(&without_test_modules(product))
            .filter(|(_, code)| line_names_any_forbidden_token(code, &CONTENT_PARSING_TOKENS))
            .count(),
        1,
