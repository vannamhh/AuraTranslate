...
- `library-index.db` và `meta.json` là DẪN XUẤT: chỉ `Indexer` ghi `library-index.db`, và chỉ sau khi `.atproj` đã ghi xong. Xoá chúng phải luôn là thao tác an toàn. Cổng canh luật này (Story 5.2): `src-tauri/tests/library_index_boundary.rs` (chỉ `core/library/indexer.rs` + điểm khai `core/store/mod.rs` được nhắc `StoreSpec::library_index`/`StoreKind::LibraryIndex`) và `src-tauri/tests/library_index_contract.rs` (hành vi xoá-và-dựng-lại, phát hiện trùng `work_id`, `.atproj` không bị chạm). ⚠️ Hai giới hạn hôm nay chưa đóng, ghi ra để người đọc AGENTS.md không phải tự suy: chỉ mục là ẢNH CHỤP lúc tạo (`updated_at`/`chapter_count` đóng băng theo `meta.json` — nợ ở `deferred-work.md`, chủ Story 5.5), và trùng `work.id` được PHÁT HIỆN + ghi chẩn đoán nhưng chưa có bề mặt HIỂN THỊ cho người dùng (chủ Story 5.6).
