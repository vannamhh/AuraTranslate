...
- `pre-push` always runs the 12 gates; it skips vitest, build and `cargo test` when every pushed file is documentation (`*.md` except the root `AGENTS.md`, which `naming_boundary.rs` reads; `_bmad-output/**` except `*-ban-do/`). Never `--no-verify`.
