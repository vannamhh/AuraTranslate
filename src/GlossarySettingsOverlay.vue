 ...
 *
 * Bảy nơi dùng cùng khuôn `padding-left: 12px` + `border-left: 2px` (nét dẫn thị
 * giác): `.gs-alert` đây · `ShortcutsOverlay.vue` `.sc-alert`/`.sc-note` ·
 * `AttributionOverlay.vue` `.attr-note` · `panels/LookupPanel.vue` `.lookup-disagree`/
 * `.lookup-row` · `panels/LookupRecord.vue` `.lookup-citation`. Đổi giá trị này mà không
 * đổi cả bảy sẽ làm lề quang học lệch giữa các câu cạnh nhau.
 */
.gs-alert {
  margin: 0 0 var(--space-panel-block) 0;
  padding-left: calc(var(--space-unit) * 3); /* = 12px, xem chú thích trên */
