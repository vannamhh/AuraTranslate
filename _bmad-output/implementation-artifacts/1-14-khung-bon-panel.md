...
| B | Giữ panel, đặt `width: 0` | ⛔ Panel vẫn trong DOM, vẫn trong vòng focus, vẫn nhận `Tab` — *"ẩn hoàn toàn"* thành lời hứa. |
...
| B | Ẩn tab bar của dockview, giữ header của `PanelFrame` | ⛔ Ẩn tab bar là ẩn luôn affordance kéo-thả và gộp-tab của FR17. Tự dựng lại = tự viết lại dockview, đúng thứ `EXPERIENCE.md:21` cấm. |
...
| C | `localStorage` | ⛔ Bị `kinds.rs:212` gọi tên là đường sai. |
...
| B | Chốt rằng **mượn là đúng**, ghi vào `DESIGN.md` | ⛔ Dev ⛔ không sửa `DESIGN.md` *(tiền lệ Ice)*, nên đường này để món nợ mở tiếp **và** để nguyên rủi ro đã ghi: *"`--weight-read-title` đổi giá trị thì hai chỗ này đổi theo mà ⛔ không ai biết."* |
...
- [x] **Task 3 — `src/layout/` — vỏ dockview và hai preset** (AC1, AC5, AC6)
  ...
  - [x] Preset mặc định **2×2**: `Nguyên văn | Bản dịch` trên, `Tra cứu | Đề xuất AI` dưới *(UX-DR13 — ⛔ đúng thứ tự này)*.
...
- [x] **Task 9 — Ba món nợ cổng** (AC11 mục 1–3)
  ...
  - [x] 🔴 Nghiệm thu **ĐỎ trước, XANH sau** cho từng phép kiểm mới — ⛔ một cổng chưa từng đỏ là một cổng chưa từng canh. Ghi bảng ca vào §Debug Log References *(khuôn: Task 10 của Story 1.6, Task 3 của Story 1.4)*.
...
| `:35` | Tệp `.ts` mang chuỗi tiếng Việt và ⛔ không cổng nào canh. *"⛔ Hệ quả: dời một chuỗi từ `.vue` sang `.ts` là cách hợp lệ về mặt cổng để cho xanh — đừng dùng."* | **⛔ KHÔNG dùng đường đó**; nợ giữ nguyên cho 10.9 |
