...
3. **Hình dạng bản ghi phủ đủ sáu trường của FR28** — nguồn · từ loại · nghĩa · ví dụ[] · trích dẫn[] · ghi chú. Lược đồ cụ thể ở §Quyết định #2; không dev không tự thiết kế lại, và không cũng không được bỏ bớt bảng nào vì *"nguồn hôm nay chưa có dữ liệu cho nó"*.
...
| **Cổng `DictionarySource`**, adapter, giữ nguyên bất đồng lúc chạy | ❌ **Không** — **Story 1.13**. không `ports/mod.rs` giữ nguyên 5 dòng |
...
| Nới CSP (`connect-src`, thêm `asset:`) | ❌ **Không** — không và không bao giờ. Gỡ scope làm mâu thuẫn biến mất **theo chiều siết**, đó là toàn bộ lý do chọn đường này |
...
**Luật:** `rebuild` cả ba bảng, và nghiệm thu bằng một truy vấn `MATCH` trả **khác rỗng** trên tệp thật — không một test chỉ khẳng định bảng *tồn tại* không bắt được ca này.
...
by_headword.entry(hw).or_insert(sense);   // không nguồn thứ hai bị NUỐT, không lỗi
```
...
3. `store_boundary.rs` cấm token `rusqlite` ngoài `src/core/store/**`. ⚠️ Phép quét đó **chỉ chạy trên `src-tauri/src/**`** (`store_boundary.rs:44`), nên `tools/` nằm ngoài — điều này là **đúng và cố ý**, không nhưng đừng vì thế mà đặt build tool dưới `src-tauri/`.
...
⚠️ **Hệ quả phải nói ra:** *"CHƯA KẾT LUẬN ĐƯỢC"* là một phán quyết **hợp lệ và bắt buộc phải ghi đúng chữ đó**. Không viết *"đạt"*, không viết *"nằm trong trần"* — cùng mệnh đề không mà Story 1.3 đã áp cho `unmeasured` của AC8.
