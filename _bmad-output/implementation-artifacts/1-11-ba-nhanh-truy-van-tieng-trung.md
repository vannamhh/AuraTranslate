...
**And** phép lọc áp cho **cả** `headword` **lẫn** `headword_simp` — ⛔ bỏ vế `headword_simp` làm `国` trả rỗng, đúng Bẫy 8 của Story 1.9
...
| **Cổng `DictionarySource`** | ❌ **Không** — **1.13**. ⛔ `ports/mod.rs` giữ nguyên **0 trait** |
...
| **`Matcher` / stemming / `jieba-rs`** | ❌ **Không** — **1.12** (AD-17). ⛔ `core/matching/` giữ nguyên |
...
| Khoá trong `src/i18n/vi.json` | **16** — ⛔ phải không đổi |
