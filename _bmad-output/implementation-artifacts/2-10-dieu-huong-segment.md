...
- [ ] 0.3 Trình Quyết định #1 · #7 **sau** Task 1
- [ ] 0.4 Ghi chữ ký + đường bị loại + ngày vào §Dev Agent Record
...
- [ ] 1.1 Tự kiểm danh tính phiên **trước mọi số** (`href` · `#app` · số hàng mong đợi) — khuyết
      tật bàn đo của 2.9 là `waitForExist` không phân biệt "Chương mới đã nạp" với "Chương cũ còn đó"
- [ ] 1.2 **Quyết định #7:** `scrollIntoView({block:'nearest', behavior:'instant'})` trên một ô
...
- [ ] 1.3 **Quyết định #1:** `⌥↓` trong ô đang gõ — WebKit làm gì mặc định, `preventDefault()`
...
- [ ] 1.4 Đo **một lượt dời con trỏ** trên Chương lớn nhất có thật — đối chiếu mốc 706–770 ms
      của `deferred-work.md` (Cạm bẫy ④). **Ghi số, KHÔNG tự chấm NFR2**
- [ ] 1.5 Ghi môi trường truy nguyên được: phiên bản WebKit · macOS · rustc/cargo · Node · ngày
- [ ] 1.6 README.md của thư mục bàn đo: cách chạy + §Kết luận + §Giới hạn thật

### Task 2 — Hai vị từ thuần trong `segmentNavigation.ts` (AC: 1, 2, 7)
- [ ] 2.1 `nextSegmentId(segments, fromId)` / `prevSegmentId(segments, fromId)` — **cùng tệp,
      cùng luật** (`import type` duy nhất, không Vue/DOM). 🔴 Duyệt bằng **chỉ số mảng**, không
      `segment.ord` — xem Cạm bẫy ⑥
- [ ] 2.2 Bỏ qua `retiredAt !== null` ở cả hai vai (đích, và không chặn đường) — đối xứng với
      `nextUntranslatedId:101`
- [ ] 2.3 `isOmitted` theo Quyết định #3
- [ ] 2.4 Biên theo Quyết định #5. 🔴 **Không quay vòng** trừ khi Ice lật tường minh
- [ ] 2.5 `fromId === null` hoặc không tìm thấy ⇒ hành vi có nghĩa, không hành vi rỗng (khuôn
      `nextUntranslatedId:95-98`, và nó có lý do thật: `fromId` trỏ vào một câu vừa bị gộp mất)

### Task 3 — Hai lệnh mới (AC: 1, 2, 9)
- [ ] 3.1 `goToNextSegment()` / `goToPrevSegment()` ở `editorPanelState.ts` — **chép khuôn ba
      dòng** của `goToNextUntranslated` (`:1029-1035`): `setEditorCaret(id)` **rồi**
      `caretPlacement.value = id`. 🔴 Không đường dời con trỏ thứ hai
- [ ] 3.2 Đăng ký ở `src/commands/index.ts` (chỗ đăng ký **duy nhất**), tiêm dep qua
      `installCommands(deps)` ở `src/main.ts` — **không** trong `App.vue`
- [ ] 3.3 Khoá `command.editor.*` mới trong `src/i18n/vi.json` (phẳng, khoá chấm có tiền tố miền)
- [ ] 3.4 Phím theo Quyết định #2. Chạy `npm run check:commands` để cổng tự bác hợp âm trùng
- [ ] 3.5 🔴 Nâng `COMMAND_FLOOR` (`scripts/check-commands.mjs:263`, hôm nay **39**) — **đếm lại
...
- [ ] 4.2 🔴 Ghi `behavior: 'instant'` **tường minh** nếu đi đường (a) — **không** luật nào canh
      `scroll-behavior` trong kho này (đã đo, xem Quyết định #7)
- [ ] 4.3 Không thêm `scroll-behavior` vào CSS ở bất kỳ đâu — và 🔵 **sửa tại chỗ** hai chú thích
...
- [ ] 4.4 Nếu đường (a) cần một tên thành viên mới của `window`/`document`: `ALLOWED_GLOBAL_MEMBERS`
...
- [ ] 4.5 🔴 **Món cho Ice:** nhìn bằng mắt rằng lượt cuộn không có hiệu ứng và hàng đích nằm
      trọn trong vùng nhìn. `happy-dom` không bố cục ⇒ không đường nghiệm thu nào của dự án mô
      phỏng được vế này

### Task 5 — Báo rõ ra màn hình (AC: 6, 7)
- [ ] 5.1 Ô nhớ theo Quyết định #4, và 🔴 **thêm vào `resetEditorPanel()`**
      (`editorPanelState.ts:443`) — Cạm bẫy ⑤
- [ ] 5.2 Bảng khoá **ĐÓNG** trên `StatusBar.vue` (khuôn `CONFIRM_NOTICE_KEYS:107` /
      `REGROUP_NOTICE_KEYS:141`) ⇒ `vue-tsc` đỏ nếu thiếu một khoá
- [ ] 5.3 Khoá `panel.grid.*` mới trong `vi.json`
- [ ] 5.4 **Gỡ `console.info`** ở `commands/index.ts:1078-1081` và thay bằng đường báo thật —
      đây là chỗ AC6 đang hỏng hôm nay
- [ ] 5.5 Ca vitest cho bất biến "ai ghi một ô thì dọn ô còn lại" (khuôn
      `tests/frontend/editorRegroupNotice.test.ts`)

### Task 6 — Đóng món nợ "đi đâu khi hết Chương" (AC: 7)
- [ ] 6.1 Cài theo Quyết định #6
- [ ] 6.2 Nối tiếp vào `deferred-work.md:2837-2847` bằng `→ ✅ ĐÃ ĐÓNG <ngày> (Story 2.10)` kèm
      **cách** đóng. 🔴 **Không xoá** mục gốc. Đóng một nửa ⇒ 🟡 + liệt kê phần còn hở

### Task 7 — Nghiệm thu (AC: 1-9)
- [ ] 7.1 vitest: hai vị từ mới + ca "ba chỗ đọc trạng thái phải đồng ý" mở rộng
      (`tests/frontend/segmentNavigation.test.ts`)
- [ ] 7.2 e2e **spec riêng**, không nhét vào spec đã đầy (trần `mochaOpts.timeout` 120 s, ~5 s
      mỗi lệnh WebDriver — bài học 2.9)
- [ ] 7.3 🔴 **Đột biến mã sản phẩm** cho mỗi ca mới: gỡ một chốt ⇒ đúng một ca đỏ. Một bộ test
      xanh chưa chứng minh gì; ba lượt gần nhất đều tìm ra lỗ hổng bằng đúng cách này
- [ ] 7.4 11 cổng npm (9 đọc-tệp + `check:scope` + `check:scope:bundled` chạy tay, cần cổng 1420
      trống) · `build` · `vue-tsc` · `cargo test --locked` · e2e
- [ ] 7.5 Đối chiếu baseline §Số nghiệm thu. 🔴 `cargo test` **phải không đổi** — xem Ranh giới ①

### Task 8 — Sổ nợ và tài liệu
- [ ] 8.1 Mọi vế không nghiệm thu được ở tầng đang làm ⇒ `deferred-work.md` **kèm một chủ**.
      Không mục nào mồ côi
- [ ] 8.2 Chú thích đã hết đúng thì **sửa tại chỗ** kèm 🔵 + ngày — đặc biệt
      `commands/index.ts:1053-1065`, khối giao món nợ cho chính story này
- [ ] 8.3 `EXPERIENCE.md` chỉ sửa theo Quyết định #2 đã ký
...
🔴 **Một lệch đáng nêu, và nó KHÔNG phải lỗi của bảng story — bảng story chưa từng khai ô này.**
`COMMAND_FLOOR` là **39** trong khi số command thật là **47**. Sàn đang thấp hơn thực tế **8**
đơn vị, nên hôm nay nó **không canh được gì**: xoá tận 8 command vẫn cho cổng xanh. Task 3.5 nói
*"nâng `COMMAND_FLOOR`"* — con số đúng sau khi thêm hai lệnh là **49**, không phải `39 + 2 = 41`.
Đây đúng là cái bẫy Cạm bẫy ⑦ mô tả *(một sàn sai thì im lặng)*, và nó **đang sống**, không phải
một khả năng lý thuyết. ⇒ Đề xuất kèm Quyết định #2: nâng thẳng lên **49**.
...
**Còn treo, chờ Task 1 (bàn đo WKWebView thật):** Quyết định **#1** *(`⌥↓` trong vùng gõ)* và
Quyết định **#7** *(cơ chế cuộn)*. Nêu trước, ký sau — đúng §0.8 mục 2.
