...
|---|---|
| **Đường IPC tra cứu từ điển** — hai pha, `DictLayers` lấy từ state, hàm thuần + vỏ mỏng | Cổng thứ tư *(AD-2 — không cấm)* · viết lại truy vấn *(đã có ở 1.11/1.11b/1.13)* |
| Hình dạng bản ghi **trên dây** *(Quyết định #2)* | Chuỗi hiển thị từ Rust *(AD-21 — không cấm)* |
...
| Ba token `lookup-*` lần đầu có người tiêu thụ — đóng nửa Lookup của `deferred-work.md:129`/`:131` | Đổi bảng token màu · đổi cờ `wraps` của `ui-md` *(không **quyết định của Ice** — `deferred-work.md:115`, lần thứ **TƯ**)* |
...
`package.json` *(**0 phụ thuộc npm mới** — NFR15)* · `src-tauri/Cargo.toml` *(không chốt lần
...
⚠️ **MỘT ngoại lệ tường minh, và không chỉ một:** khối `<style>` của `SourcePanel.vue` và
...
**✅ ĐỤNG ĐÃ ĐƯỢC PHÉP** *(và không chỉ khi Ice chốt Quyết định tương ứng)*:
...
> Khuôn Task 0 của Story 1.13/1.14/1.15/1.16. Mỗi mục có **mặc định đề xuất** kèm lý do; không
...
⚠️ **Và luật SQL không nhân nhượng:** `LIMIT` là **tham số ràng buộc** (`LIMIT ?N`), không
...
`NoBranchQueryTooShort` **không** chuẩn bị câu SQL nào — `LIMIT` không áp cho nó, không
...
| `📌 Ghim ⌘D` | không | Story **1.20** |
| Dải âm đọc `dǎ · đả — 21 nghĩa` | không | Đòi `dict_entry.reading` *(chưa qua dây)* **và** một luật lọc nghĩa theo âm *(không FR nào)* |
| Chip lọc từ loại `động từ 17` | không | Không FR nào đòi. Nhặt lại khi có |
...
sang token mới *(cỡ đổi)*, hay khai thêm một biến thể `ui-sm-wrap`. **Ghi ra lựa chọn**, không
...
**And** không trường vắng mặt (`pos = None`, `note = None`, `examples = []`) ⇒ **không render
...
**And** không **chỉ một** nguồn có kết quả ⇒ **không** dòng dẫn *(một nguồn không bất đồng
với ai)*
...
**And** không đây **không** phải một lời gọi thứ hai rải ra — nó đi qua **đúng điểm nghẽn** mà
...
**And** ⚠️ xét điều kiện *"nếu cần"* **trước**: đầu-cuối **< 100 ms** ⇒ **NFR1 đã đạt** ⇒ không
...
**And** không `matchMedia` = **0**, `window.innerWidth` = **0** trong `src/**` *(giữ nguyên)*
...
- [x] **Task 3 — Pha hai đi qua ĐÚNG lớp** *(AC: 8)*
  ...
  - [x] `senses(&[])` ⇒ không chạm database *(cùng luật `han_viet(&[])`)* *(chứng minh gián
        tiếp qua `lookup_command_calls_senses_with_an_empty_batch_for_no_layer` — không nhóm nào
        khớp ⇒ `senses_by_layer` rỗng, không một khoá lớp nào có mặt)*
...
- [x] **Task 5 — Vùng đầu mục chiều cao cố định + thanh nhịp** *(AC: 7; Quyết định #6)*
  - [x] Hằng có tên cho chiều cao; không con số rải trong CSS *(`--lookup-head-height: 76px`
        + `overflow: hidden` trên `.lookup-head` — điều kiện CƠ HỌC "không đổi một pixel" kể cả
...
- [x] **Task 6 — Khối nguồn và bản ghi** *(AC: 1, 2, 3, 4, 5)*
  ...
  - [x] Nhiều đầu mục cùng `headword` trong một nguồn — theo Quyết định #5 *(danh sách
        ...
        component không cần biết ranh giới đầu mục)*
...
- [x] **Task 7 — Token, token thứ 17, và cổng thị giác** *(AC: 9; Quyết định #7 — ✅ đã chốt)*
  ...
  - [x] ⚠️ `.parallel-note` đang ở `ui-sm` (11,5px) — chọn: đổi sang `ui-md-wrap` *(**cỡ chữ
        ...
        sang `ui-md-wrap` — 11,5px→12px, dùng CHUNG một token cho cả ba chỗ, không thêm
...
- [x] **Task 8 — 🔴 ĐO NFR1 đầu-cuối, TRƯỚC và SAU `LIMIT`** *(AC: 11, 12)*
  ...
  - [x] 🔴 **Nếu `LIMIT` không cắt được thời gian ⇒ DỪNG THI HÀNH** *(Ice chốt)* — **không áp
        dụng**: `LIMIT` ĐÃ đo được là cắt thời gian thật (Task 0), nên không cần khảo sát bốn
        ...
        thẳng vào §Debug Log References, không giấu.
  - [x] Ghi giới hạn của phép đo *(engine nào, có tệp `.db` thật hay không)* — không giấu
        *(backend: dữ liệu thật + hàm sản phẩm thật; webview: Chromium thật nhưng IPC giả
        lập tức thời, không có bản Tauri đã đóng gói để đo vòng IPC thật — ghi rõ trong §Debug
        Log References)*
...
- [x] **Task 9 — `vi.json` + chuỗi** *(AC: 6; NFR16)*
  - [x] Bốn+ chuỗi trạng thái rỗng, mỗi trạng thái **một chuỗi riêng** — không dùng lại
        *(sáu khoá mới `panel.lookup.*` + tái dùng `panel.lookup.status` sẵn có cho ca
        "chưa tra gì" — đúng nghĩa gốc của khoá đó, không dùng lại chồng nghĩa)*
  - [x] không Chuỗi *không tìm thấy* **không** nhắc *Concordance* *(đã đối chiếu nguyên văn)*
...
- [x] **Task 11 — Bàn giao trung thực**
  ...
  - [x] `deferred-work.md`: đóng những mục story này đóng thật; **ghi mới** những gì chưa đo
        *(mười một mục đóng: `:453` `:129/:131` `:133` `:115` `:343` `:363` `:416` `:419/:449`
        `:504` — cộng mục `:317` (mục từ tiếng Anh) ghi rõ không đóng; mục mới "Deferred from:
  ...
  - [x] **Không** đánh dấu đạt cho bất cứ thứ gì chưa chạy — vế thị giác hai nền tảng
        ...
        *(ghi rõ trong deferred-work.md: story đo bằng Rust thật + Playwright/Chromium thật,
        WKWebView/WebView2/`tauri dev` thật, không tự nhận đã đóng món nợ đó)*
...
- [x] [Review][Patch] **Lỗi IPC ⇒ thân panel TRẮNG CÂM; `lookupError` là một export không ai tiêu thụ** [src/panels/lookupPanelState.ts:86 · src/panels/LookupPanel.vue:17-28] — 🔴 **nặng nhất.** `grep lookupError src/**` cho **0 chỗ import**. Khi `lookupDictionary()` trả lỗi: `neverLookedUp === false` ⇒ `PanelFrame` tắt câu mặc định; `lookupResolved === false` ⇒ cả khối kết quả lẫn **cả bốn** chuỗi rỗng đều không render. Người dùng nhận đúng một dòng đầu mục và không một câu nào giải thích. `vi.json` cũng không có khoá lỗi tra cứu nào. Vi phạm AC6 · AD-44 ④ · UX-DR27 — và là **tái phát nguyên văn** khuyết tật mà code review 1.16 đã bắt *(bằng chứng còn trong cây: `SourcePanel.vue:56`)*, đúng thứ §Trí tuệ từ story trước dán vào để không học lại bằng tiền.
- [x] [Review][Patch] **Kết quả cũ bị xoá trắng trong khoảng chờ, banner cũ thì ở lại — và chú thích tại chỗ nói NGƯỢC với mã** [src/panels/LookupPanel.vue:34-37,72-73 · src/panels/lookupPanelState.ts:130,136,139,142] — chú thích `:34-37` khẳng định *"panel giữ nguyên trạng thái TRƯỚC ĐÓ cho tới khi lượt tra mới trả lời"*, nhưng `groupedLookup`/`sensesByLayer` cổng qua `lookupResolved` (`false` ngay khi `pending`) ⇒ toàn bộ bản ghi biến mất. Cùng lúc `someLayerFailed`/`someLayerTruncated` đọc thẳng `response.value` **không qua** `lookupResolved` ⇒ banner của truy vấn **CŨ** nổi trên vùng bản ghi trống, dưới đầu mục **MỚI**. Đúng lớp nháy mà AC7 tồn tại để chặn.
- [x] [Review][Patch] **`runLookup` không có số thứ tự lượt; `resetLookupPanel` không huỷ lượt đang bay** [src/panels/lookupPanelState.ts:152-162,172-177] — `query.value` đặt **trước** `await`, `response.value` đặt bởi *bất kỳ* lượt nào về trước. Hai lần `Mod+Alt+L` liên tiếp: nếu lượt A về sau lượt B ⇒ đầu mục B kèm bản ghi A, `pending` đã tắt nên `lookupResolved === true` — không dấu hiệu nào báo sai, và trạng thái đó **vĩnh viễn**. Cùng lỗ: đổi Tác phẩm trong lúc một lượt đang bay ⇒ promise ghi **sau** `resetLookupPanel()` làm kết quả Tác phẩm A sống lại dưới Tác phẩm B — đúng thứ reset tồn tại để chặn (Bẫy 8). Story 1.18 (Auto-Lookup) sẽ biến lỗ này thành thường trực.
- [x] [Review][Patch] **`QUERY_LENGTH_CEILING` cắt truy vấn IM LẶNG ⇒ "Không tìm thấy trong từ điển" là một câu SAI** [src-tauri/src/commands/dict.rs:59,94] — bôi đen > 200 ký tự bị cắt còn 200 rồi tra `Exact` ⇒ chắc chắn 0 kết quả ⇒ panel hiện `panel.lookup.not_found`, trong khi hệ thống **chưa hề tra** thứ người dùng chọn. Story đã dựng đúng cơ chế cần thiết (`truncated`/`truncated_layers`) cho trần số HÀNG mà không áp cùng nguyên tắc cho trần độ DÀI. Sửa: thêm một cờ vào `LookupResponse` *(vỏ lệnh, không đụng hình dạng bản ghi của Quyết định #2)* và một chuỗi riêng.
- [x] [Review][Patch] **`limit as i64` biến một trần lớn thành `LIMIT 0` — mất sạch dữ liệu, im lặng, ở một hàm `pub`** [src-tauri/src/core/dict/query.rs:141,197,301] — `(limit as i64).saturating_add(1)`: với `limit = usize::MAX` *(thành ngữ tự nhiên nhất cho "không giới hạn")* kết quả là `-1 + 1 = 0` ⇒ SQL trả 0 hàng, và `cap()` (`:84-88`) báo `truncated = false` vì `0 > usize::MAX` sai. `saturating_add` đang bảo vệ **sai chỗ** — phép ép kiểu đã tràn trước đó. Bằng chứng gián tiếp: cả hai tệp test phải bịa `const UNLIMITED: usize = 10_000` thay vì dùng `usize::MAX`, tức *"không giới hạn"* không biểu đạt được trong API này và mọi số đo "trước `LIMIT`" thật ra chạy với `LIMIT 10001`. Cùng hàm: `limit == 0` cho `groups` rỗng **kèm** `truncated = true` ⇒ panel hiện đồng thời "không tìm thấy" và "danh sách chưa đầy đủ". Sửa: `i64::try_from(limit).unwrap_or(i64::MAX)` + chặn `limit == 0`.
...
- [x] [Review][Patch] **`LOOKUP_PAGE_LIMIT` không được ghim bởi một ca test nào — có thể tụt xuống 1 mà cả bộ vẫn xanh** [src-tauri/src/commands/dict.rs:51 · src-tauri/tests/dict_sources.rs] — hai ca AC12 gọi `lookup_grouped(..., 3)` với trần truyền **tay**, không đi qua `commands::dict::lookup`. Các ca đi qua command dùng fixture ≤ 1–2 đầu mục mỗi lớp, nên đặt `LOOKUP_PAGE_LIMIT = 1` vẫn xanh toàn bộ. Hằng mang **cả chính sách sản phẩm của Quyết định #4** lại là thứ duy nhất trong story không có lưới hồi quy.
- [x] [Review][Patch] **Thanh nhịp render `"7 · 22"` — mất chữ "nguồn"/"nghĩa"** [src/panels/LookupPanel.vue:62 · src/i18n/vi.json] — mockup `lookup-real-density.html:116` là `5 nguồn · 22 nghĩa`; Completion Notes tự xác nhận kết quả nghiệm thu bằng mắt là `"2 · 3 CVDICT 2 THIỀU CHỬU 1"` — một dãy số không đọc được, trong khi lý do tồn tại của thanh nhịp là *"biết hình dạng trước khi cuộn"*. `vi.json` không thêm khoá đơn vị nào ⇒ vế này chưa làm chứ không phải làm khác.
- [x] [Review][Patch] **Vết phê duyệt và File List lệch sổ** [Change Log · §Project Structure Notes] — Completion Notes viện dẫn *"**Ice xác nhận (b)**"* cho việc AC12 chấp nhận nguồn thứ hai vắng mặt, nhưng Change Log *(nơi mọi lượt chốt của Ice đều có một hàng ngày tháng)* không có hàng nào ghi lượt đó. Đường (b) **được** nhánh `Or` của AC12 cho phép nên đây không phải vi phạm — nhưng vết phê duyệt cần có mặt trước khi story rời `review`. Cùng hạng: `src-tauri/tests/ipc_contract.rs` được §Project Structure Notes ghi `UPDATE` mà không được đụng.
- [x] [Review][Defer] **Chip thanh nhịp bị `overflow: hidden` cắt CÂM khi nhiều nguồn** [src/panels/LookupPanel.vue:109-113,131-137] — deferred, đánh đổi có chủ đích đã ghi trong chú thích tại chỗ *(giữ bất biến AC7 quan trọng hơn)*; đo thật `山` cho **7–8 nhóm** ⇒ chip tràn quá hai dòng bị cắt, không dấu hiệu, không cuộn.
- [x] [Review][Defer] **`layers_loaded = false` khi MỌI tệp `.db` hỏng ⇒ hiện "chưa gắn lớp nào" (sai)** [src-tauri/src/core/dict/layer.rs:460-493 · mod.rs] — deferred, ca hiếm; `DictLayers::new` đẩy lớp mở hỏng vào `skipped` chứ không vào `layers`, nên `layers().is_empty()` đúng về cơ học mà sai về chẩn đoán. Banner `someLayerFailed` vẫn hiện song song nên người dùng không bị bỏ câm.
- [x] [Review][Defer] **Pha hai trượt ⇒ khối nguồn hiện tên với 0 nghĩa, không phân biệt được với đầu mục chỉ có âm đọc** [src-tauri/src/commands/dict.rs:109-118] — deferred, `else { continue }` + `senses(...).unwrap_or_default()` nuốt lỗi hydrate sau khi pha một đã thành công; đã được ghi là món nợ ở deferred-work *("`senses()` trượt không có tín hiệu riêng")*.
...
- [x] [Review][Defer] **Nhánh `Substring`/`fts_trigram` nạp TOÀN BỘ hàng khớp vào RAM trước khi cắt** [src-tauri/src/core/dict/query.rs:203-221,238-254,321-333] — deferred, latent: đường sản phẩm 1.17 là `Exact`-only nên không chạm tới; trở thành thật khi 1.18/7.7 bật `Substring`. Trần an toàn ở SQL (vd `limit * 50`) vẫn giữ được thứ tự "verify rồi mới `cap`".
- [x] [Review][Defer] **Chuỗi `query_too_short` chỉ dẫn một thao tác không tồn tại trong panel** [src/i18n/vi.json:65] — deferred, `"gõ thêm ít nhất ba ký tự"` nhưng Panel Lookup không có ô nhập nào; story tự khai nhánh này không thực thi được qua đường sản phẩm `Exact`-only hôm nay. Story 1.18/7.7 sẽ kế thừa nguyên văn.
...
2. **Gộp/khử trùng ở tầng webview** ⇒ AD-19 vỡ. `no_function_merges_meanings_across_sources`
   **đếm tệp Rust**, không hoàn toàn mù với `src/**`. Một `new Set(...)` trên `gloss` là đủ.
...
### 🔴 BỐN mâu thuẫn tài liệu đã phát hiện — không dev KHÔNG sửa tài liệu, chỉ NÓI RA
...
4. 🔴 **Vạch trái 2px + thụt 13px treo ở CẤP NÀO — hai tài liệu nói hai cấp.** `epics.md`
   ...
   làm mắt nhặt được ranh giới nghĩa)* **hoặc** theo `epics.md`; không dù chọn bên nào cũng
...
- **Story 1.14:** trạng thái tiêu điểm đọc từ **DOM thật**, không từ `activePanel`. không
...
src/
  panels/LookupPanel.vue        UPDATE  không thay doc-comment "khung, không phải nội dung"
...
1. ✅ **Mục từ TIẾNG ANH — TẠM DÙNG MẶC ĐỊNH.** `deferred-work.md:317` mở từ 2026-08-05, chủ
   ...
   **tiếng Trung**; mục tiếng Anh có hình dạng khác *(từ loại + nghĩa tiếng Việt + ví dụ, không
...
🔴 **ĐO LẠI 2026-08-07 (code review, Ice chốt "đo lại trước khi đóng") — p99 70,742 ms không TÁI LẬP.**
...
≈ **101 ms** — chạm/vượt trần NFR1 100 ms. Ice chốt: không đóng story trên một con số chưa
hiểu (*"nghi không phải là biết"*).
...
⇒ **70,742 ms là NHIỄU của lượt đo gốc, không một thuộc tính của mã.** p99 đường sản phẩm ổn
định ở **0,57–4,04 ms** qua ba lượt — cách trần 100 ms hai bậc độ lớn, và không còn khoảng
...
tệp `.db` chưa được OS ánh xạ — đúng thứ `WARMUP` tồn tại để loại mà 10 vòng không đủ cho một
tệp 592.538 đầu mục đọc lần đầu.

**⇒ NFR1 ĐẠT, và lần này kết luận đứng trên p99 chứ không chỉ p95.** Ước tính đầu-cuối xấu
nhất: 4,04 ms (backend p99) + ~31 ms (render, Playwright/Chromium thật) ≈ **35 ms**, trần
100 ms.

⚠️ **Giới hạn phép đo không đổi:** vòng IPC Tauri thật (WKWebView/WebView2) vẫn **chưa** được
đo — xem §giới hạn bên dưới. Ba lượt trên chỉ khoá lại vế **backend**.

⚠️ **Một ngoại lai KHÁC vẫn còn, và nó không nằm trên đường sản phẩm:** `en-2-trigram/tất cả`
(pha hai hydrate **toàn bộ** 572 đầu mục) đo được p99 **57,771 ms** ở lượt #1. Nhánh này
không được `commands::dict::lookup()` chạm tới (nó hydrate **một trang**, không toàn bộ — đúng
Bẫy 4), nên nó không ảnh hưởng NFR1 hôm nay. Ghi ra vì Story 1.18/7.7 sẽ mở đường `Substring`
...
3. Số đo char_idx "sau LIMIT" (`LIMIT=20`, gọi trực tiếp `lookup_grouped` vì đường Substring không qua command hôm nay) vẫn cho kết quả tốt: p95 **5,109 ms** so với **20,836 ms** không `LIMIT` — cơ chế hoạt động đúng như thiết kế, sẵn sàng cho chỗ gọi tương lai.
...
**Đo webview (Playwright headless Chromium, mock `window.__TAURI_INTERNALS__.invoke` trả lời tức thời):** thời gian từ dispatch phím `Mod+Alt+L` tới khi `.lookup-source-name` xuất hiện trong DOM — 15 lượt, p50 ≈ 31 ms, khoảng 11,6–111,2 ms (nhiễu cao, đặc trưng của môi trường headless test + chi phí dispatch sự kiện của Playwright, không đại diện chính xác một cú gõ phím thật). Đây là ước tính CHO PHẦN RENDER, không phải số đo "đầu-cuối" hoàn chỉnh.
...
- Webview render: đo qua **Chromium thật** (Playwright) nhưng với IPC **giả lập tức thời** (`invoke` mock trả lời ngay, không có độ trễ round-trip thật của Tauri) và trong môi trường headless CI-like — số liệu chỉ mang tính ƯỚC LƯỢNG cho phần render, không phải phép đo webview thật trên bản build Tauri đã đóng gói.
- **Không có** một bản Tauri đã biên dịch + WKWebView/WebView2 thật đang chạy trong phiên này để đo vòng IPC THẬT (serialize Rust → deserialize JS → Vue reactivity → paint) — đây là món nợ cũ (thị giác hai nền tảng thật), Task 11 ghi lại nguyên trạng, không đánh dấu đạt cho vế đó.
- **Kết luận có cơ sở nhưng KHÔNG phải một phép đo đầu-cuối hoàn chỉnh 100%:** backend p95 6,535 ms + JSON 35 KB (serialize nhanh, không đo riêng được vì `to_string` trong Rust chạy < 1ms cho kích cỡ này) + render ước lượng ~30 ms ⇒ tổng ước tính **dưới 40 ms**, cách xa trần 100 ms của NFR1 với biên độ lớn — nhưng vòng IPC Tauri thật (serialize/deserialize qua cầu JS) chưa được đo trực tiếp.
