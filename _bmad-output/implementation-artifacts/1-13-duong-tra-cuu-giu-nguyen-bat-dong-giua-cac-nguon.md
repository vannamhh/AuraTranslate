...
> 🟡 **HAI chỗ tài liệu ĐANG LỆCH — ⛔ DEV KHÔNG SỬA. Ghi ra để ⛔ không ai tưởng mình đọc nhầm.**
...
`deferred-work.md` *(Story 1.10, ⛔ vẫn mở, chủ sở hữu ghi đích danh là **1.13**)*: `dict-vietphrase.db` chứa **18** đầu mục trùng *(46 trong nguồn thô)*. Tra `不是他的对手` trả **HAI** `dict_entry` **từ CÙNG một nguồn** ⇒ Panel Lookup của 1.17 hiện **hai khối "VietPhrase" giống hệt nhau**.
...
**And** phiên bản đọc từ `PRAGMA user_version`; nếu `dict_meta('schema_version')` **⛔ không khớp** nó ⇒ lớp bị **từ chối** với lý do riêng *(hai chỗ ghi phiên bản là chủ ý của Story 1.9 §Quyết định #2; hai chỗ **⛔ nói khác nhau** nghĩa là tệp ⛔ không do `tools/dict-build` viết ra, và tin nửa nào cũng là đoán)*
...
- **1.12:** cổng ranh giới là **phép quét CHỮ**, ⛔ không phải phân tích đồ thị gọi hàm — một re-export đổi tên lách được. Giới hạn **có sẵn** của khuôn, ⛔ không phải thứ story này phải giải; ⛔ nhưng cũng đừng dựa vào cổng như thể nó ngữ nghĩa.
...
src-tauri/src/
  core/dict/
    ...
    query.rs      # ⚠️ có thể SỬA — thêm SQL đọc nghĩa; ⛔ giữ nguyên năm hàm nhánh đã có
