...
**And** **đầu ra THẬT** ghi vào Completion Notes thành bảng, kèm câu trả lời: chuỗi thật **có** trùng `dictionari` · `studi` · `happi` ⛔ hay không
...
- [x] **Task 1 — Khung module và hằng cấu hình** *(AC1, AC3, AC4, AC9)*
  - [x] 1.1 Viết doc-comment module `core/matching/mod.rs` theo phong cách sẵn có của `core/dict/mod.rs`: nêu AD-17, nêu **ranh giới ⛔ `dict/` không gọi** kèm lý do đo được, nêu hai người tiêu thụ tương lai *(3.4 · 7.6)*
...
///
/// ⛔ Đây **KHÔNG** phải `core::dict::QueryRoute`. `QueryRoute` trả lời *"tra vào bảng nào
...
| `ed8ce52` / `a3ed5cd` | Khuôn **fixture dựng trong test** + integration test — cùng triết lý `matching_contract.rs` dùng, ⛔ trừ phần fixture *(story này ⛔ không cần `.db`)* |
...
| `…的文化` | `的` · `文化` | `文` | ⛔ **từ chối** — đầu cuối rơi **giữa** token |
...
#### ⑩ Ba việc bàn giao ra ngoài story — đã ghi `deferred-work.md`, ⛔ dev không tự sửa
