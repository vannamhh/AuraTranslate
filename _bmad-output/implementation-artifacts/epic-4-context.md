...
- 🔵 **FIXED 2026-09-22 (Story 4.10).** This line read "Two mockups ... cover this epic", naming only `prompt-library.html` (Stories 4.4–4.5) and `prompt-inspector.html` (Story 4.7). Counted on disk: **three**. The missing one is `ai-batch-translate.html`, which covers FR73 · FR74 · FR75 · FR76 — that is Stories 4.9, 4.10 and 4.11, the three stories the original line left without a mockup. Its FR75 row draws the error screen in detail, including copy and three actions, and Story 4.10 planned against it. All three exist in `planning-artifacts/ux-designs/ux-AuraTranslate-2026-08-02/mockups/`. They are reference, not specification — where a mockup and a signed decision disagree, the decision wins, and Story 4.10's Decision 1 is the worked example: that mockup's "Đưa 5 câu đã xong sang" button stays unbuilt because a measured 🔴 in spec 4.9 forbids writing more than one segment per user action.
