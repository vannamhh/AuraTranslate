...
- `.githooks/pre-push`: green, eleven gates + vitest + build + `cargo test`, 161 s.
  🔴 Ice's macOS only — per `AGENTS.md` it says nothing about Windows or UTC. **The CI run has
  not been read: nothing has been pushed. This spec is not done until it is.**
