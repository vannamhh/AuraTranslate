...
> 🟡 **`epics.md` ĐANG LỆCH khỏi AD-44 ở hai chỗ — không DEV KHÔNG SỬA `epics.md`. Chủ sở hữu: John (PM).**
...
**Then** trả `QueryRoute::Zh` nếu chuỗi chứa **bất kỳ** ký tự Hán nào; không ngược lại trả `QueryRoute::En`
...
9. 🟡 **không Chuỗi tiếng Việt CÓ DẤU ở vị trí mã bị `check-i18n.mjs` Kiểm A bắt.** `src/core/dict/**` **không** nằm trong `EXEMPT`; chỉ `src-tauri/tests/**` được miễn. **Doc-comment và comment có dấu thì hợp lệ**; `panic!` / `debug_assert!` / `format!` / `Display` thì **không**. *(Đã bắt đúng ca này ở `core/i18n/mod.rs` và ở một **tên test** của 1.10b.)*
...
```text
core/
  matching/      # jieba + stemmer — DÙNG CHUNG (AD-17)          ← không story này KHÔNG chạm
...
- [Source: `tools/dict-build/src/schema.rs`] — DDL *(không chỉ đọc, không sửa)*
...
- [x] **`cargo fmt` — baseline đỏ RỘNG HƠN story ghi; tệp story này chạm thì SẠCH.**
  ...
  **Không** đụng một chỗ nào trong số đó. Bốn tệp story này chạm —
  ...
  chỗ nào** *(chạy `rustfmt` riêng cho hai tệp test; hai tệp `src` vốn đã sạch)*. CI không
...
- [x] **Cổng cuối — tất cả xanh, không hạ một sàn nào.**
  ...
  nới — `core/dict/**` không vẫn không gõ `rusqlite`. Sàn **mới** `SRC_TAURI_RS_FLOOR = 20` là
  một sàn **thêm vào**, không phải một sàn bị hạ.

  **Ranh giới KHÔNG CHẠM, đã đối chiếu bằng `git status`:** 0 dòng `.vue`/`.ts`/`.css`
  · không `src-tauri/Cargo.toml` và `Cargo.lock` không đổi một dòng · không `tools/dict-build/**`
  không chạm · **không** lượt `cargo run` nào của `dict-build` · không `ports/`,
