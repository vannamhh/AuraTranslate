...
- [x] **Task 2 — `scope_kinds!` và bảng ngữ nghĩa** (AC2, AC3, AC4)
  ...
  - [x] `pub enum Tier { Global, Work }` — không tên là `Work`, **không** `Project`. Consistency Conventions cấm `Project` cho thực thể Tác phẩm; `StoreKind::Project` đặt tên cho **tệp** `project.db`, không cho tầng.
...
- [x] **Task 4 — `ScopeResolver` là điểm vào duy nhất** (AC1)
  ...
  - [x] không `core::scope` **không `use tauri::…`** — cùng lý do Quyết định #1 của Story 1.7: test dựng được trên thư mục tạm mà không cần webview. Đường lấy `AppHandle`/`State` nằm ở `commands/`.
...
- [x] **Task 6 — Đường đọc/ghi tầng Global** (AC5)
  ...
  - [x] không `core::scope` **không** sở hữu kho của mọi loại. Glossary/TM/Prompt/Luật làm sạch sẽ mang **bảng riêng của chúng** ở epic của chúng; `config_value` chỉ phục vụ ba loại `GlobalOnly` (§Quyết định #5).
...
`.atproj` là **Story 1.15**, bảy story nữa. `src-tauri/src/core/store/mod.rs:122-134` nói thẳng: *"không Chỉ [`StoreKind::Global`] có mã khởi tạo hôm nay… Viết sẵn mã cho hai loại kia hôm nay là mã không ai gọi, và nó sẽ sai theo đúng cách mà không test nào bắt."*
...
- Frontend **không có test runner** và không story này không được thêm một cái (NFR15 đòi rà giấy phép trước). Hành vi frontend nghiệm thu bằng các cổng `.mjs` sẵn có.
...
- `ARCHITECTURE-SPINE.md#AD-7` — năm loại kho · `#AD-8` — không cấu hình không nằm ở `library-index.db` · `#AD-9` — `.atproj` là thư mục
...
- `src-tauri/src/core/store/mod.rs:98-117` *(tái xuất)* · `:238` `StoreSpec::global` · `:433-456` `From<StoreError> for IpcError` · `:556`/`:576` `write`/`read` · `:122-134` không chỉ `Global` có mã khởi tạo
...
- `src-tauri/tests/store_contract.rs:752-770` 🔴 — test **duy nhất** chạy trên `GLOBAL_MIGRATIONS` thật; `:714` doc-comment thành sai; `:718-741` `TWO_STEP`/`BROKEN_STEP_TWO` không giữ nguyên; `:921` tự đúng theo
- `src-tauri/tests/ipc_contract.rs:73-78,128-137` 🔴 — mệnh đề vòng phải chữa
- `src-tauri/tests/config_invariants.rs:333,368` — không `capabilities/main.json` bị khoá đúng ba quyền
...
**Sửa — test Rust (3)**
- `src-tauri/tests/store_contract.rs` — **đúng một** ca (`a_fresh_database_migrates_up_to_target_and_logs_it`) + doc-comment `:714` đã thành sai; không `TWO_STEP`/`BROKEN_STEP_TWO` giữ nguyên
