...
| Method **thứ ba** trên cổng `DictionarySource` — đọc `dict_entry.han_viet` theo **lô ký tự** | Cổng thứ tư *(AD-2 — không cấm)* · đường đọc `dict_sense` *(đã có ở 1.13)* |
...
| `font-synthesis: var(--synthesis-source-hanviet)` — người tiêu thụ **đầu tiên** của lời giải chữ Hán nghiêng giả *(`deferred-work.md:133`)* | Đổi bảng token màu · đổi cờ `wraps` của `ui-md` *(không quyết định của Ice, `deferred-work.md:115`)* |
...
· `src-tauri/Cargo.toml` *(không chốt lần thứ **năm** — `deferred-work.md` [D4])* · `[profile.release]`
...
> Khuôn Task 0 của Story 1.13 / 1.14 / 1.15. Mỗi mục có **mặc định đề xuất** kèm lý do; không
...
**Vì sao (C) bị loại, và vì sao không nó không vi phạm AD-19:** AD-19 cấm *"bước **hợp nhất
...
⚠️ **Lỗ hổng đã biết, không ghi ra thay vì lấp bằng phát minh:** **không** mockup nào và không
...
🔴 **Ràng buộc bắt buộc của (a) — không ĐỌC KỸ, đây là chỗ nó phá Story 1.18 và 3.4 trong im lặng:**
...
**Mặc định đề xuất:** một **trần render có tên, hiện ra bằng một chuỗi `vi.json`**, không
...
**And** ngắt dòng của văn bản nguồn hiện ra thành ngắt dòng, và `\r` của tệp Windows không
...
và bảng đóng băng của `check-tokens.mjs` **không vẫn đúng 14 hàng**
...
nay, vì `src-tauri/resources/dict/` không **rỗng trong git** *(AD-25; `lib.rs::open_dict_layers`
...
Quyết định #4 chốt)*
**And** cả hai lệnh đổi tab lẫn đổi kiểu xem đi qua **`CommandRegistry`** và **không có phím** —
...
- [x] **Task 2 — Method thứ ba trên cổng `DictionarySource`** (AC4, Quyết định #2)
  - [x] Khai **hình dạng** ở `ports/dict_source.rs` — không tệp đó không gõ tên crate SQLite
...
1. **`Unihan kVietnamese` không phải Hán Việt thuần** — `epics.md:1693` và `prd.md` FR33
   ...
   `EXPERIENCE.md:410` còn xây ví dụ trụ cột của FR113 trên một chữ (`北`) mà lớp nền không
...
src/
  panels/SourcePanel.vue           UPDATE  không thay doc-comment "khung, không phải nội dung"
...
mỗi lượt nạp Chương (không phải đường nóng NFR1 — Quyết định #7), 13,6 s ở 500k thì không
...
- [x] [Review][Patch] **Doc-comment `sourceChapterError` nói ngược nghĩa** — viết *"`null` … khi không **có** cầu IPC"*, hành vi thật là `null` khi **KHÔNG** có cầu IPC. [`src/panels/sourcePanelState.ts:97`] — mức: **low**
