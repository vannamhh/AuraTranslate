...
> tệp đang mở · trần `MAX_PATH` · kéo-thả qua WebView2)* ⛔ **chưa từng chạy ở đâu**, và ⛔
...
| **(c) ⛔ không ghi `meta.json`, dựng nó lúc quét** | ⛔ Phá AD-9 *(Library đọc metadata **không cần mở SQLite**)* và NFR4 |

**✅ Ice chốt 2026-08-06: (b).** ⛔ (a) **đã bị loại** — ⛔ tuyệt đối không `fs::write` bên trong closure của `Store::write`. Kèm **hai ràng buộc bắt buộc**:
...
- [x] **Task 4 — Đường nhập** *(AC1, AC8; AD-39)*
  ...
  - [x] ⛔ Đường kéo-thả và ô đường dẫn nhận **đường dẫn**; ⛔ **không** để webview đọc nội dung tệp rồi đẩy chuỗi qua IPC *(AD-1 + AD-16 — mọi nội dung ngoài do **Rust** phân tích)*
...
- [x] **Task 9 — Chuỗi i18n + `MessageKey`** *(AC8, NFR16)*
  ...
  - [x] ⛔ `params` mang **dữ liệu**, ⛔ không mang **câu**. Một `params: {"reason": "<câu>"}` là AD-21 thủng qua cửa sau
...
### 🔴 Mâu thuẫn tài liệu đã phát hiện — ⛔ dev KHÔNG sửa tài liệu, chỉ NÓI RA
...
**Đường xử lý** *(⛔ dev không sửa mockup)*: viết lại ở dạng **vô nhân xưng** khi đưa vào `vi.json` — ví dụ *"Một Tác phẩm là một dự án dịch, lưu thành một thư mục trên máy. Copy thư mục đó đi đâu cũng mở lại được."* Ghi độ lệch vào §Completion Notes.
...
| 2 | **Tầng Tác phẩm của `ScopeResolver` chưa từng chạy trên dữ liệu thật.** *"`WorkScope` là một struct rỗng đánh dấu chỗ. **Story 1.15** cắm tầng thật vào; ⛔ ba chữ ký ⛔ không phải đổi"* | 🔴 Nợ PHẢI trả — AC9 |
...
**Mã**
- `src-tauri/src/core/store/{mod,schema,pragmas,writer,reader,checkpoint}.rs` — tầng ghi, ⛔ **dùng, không viết lại**
...
1. **`PROJECT_MIGRATIONS` là BA bước, ⛔ không MỘT bước gồm cả ba DDL như văn bản story gợi ý**
   ...
   **tái dùng**, ⛔ viết lại) vào một chuỗi duy nhất buộc phải chép lại nguyên văn của nó. Ba
...
2. ⚠️ **Kéo-thả tay thật bằng chuột người** vẫn chưa chạy — cơ chế nay còn nhận thêm hai event mới
   (`DRAG_ENTER_EVENT`/`DRAG_LEAVE_EVENT`), nên bề mặt cần nghiệm thu tay **rộng hơn** trước, ⛔
