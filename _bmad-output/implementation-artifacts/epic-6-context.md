...
- Xem trước đa-Chương: mỗi Chương mang dữ liệu của chính nó ở tầng ranh giới bóc lẫn tầng luật làm sạch, không mượn số Chương khác — nền bắt buộc trước khi bộ lọc bên dưới có ý nghĩa. `⌥←`/`⌥→` dừng ở hai đầu danh sách, không cuộn vòng. Link hỏng giữ chỗ đúng vị trí kèm một trong tám lý do, nhưng "xem được" và "ghi được" là hai mệnh đề khác nhau — nút xác nhận vẫn khoá tới khi xử lý xong. Không thao tác tay nào thì byte ghi xuống phải trùng đúng kết quả trước khi có xem trước đa-Chương.
- Bộ lọc "cần xem" áp cho mọi đường nhập — điều kiện để dò bảng mã/bóc nội dung/luật làm sạch còn tác dụng ở quy mô hàng chục Chương. Hai con số đầu màn (`N Chương cần xem`/`M Chương sạch`) gom **năm** nguyên nhân (nới 2026-09-08): bảng mã tin cậy thấp (cờ cấp **lượt nhập**, không lẫn từng Chương), bóc ra ngắn bất thường so với trung vị, luật làm sạch xoá quá nhiều (so trung vị), **số dòng bị bước chuẩn hoá (FR125) nối lại cao bất thường so với trung vị** (con số đã tính sẵn trên toàn văn từng Chương ở bước chuẩn hoá, chỉ cần nối dây thay vì vứt), và link hỏng. Một Chương **không đo được** một dấu hiệu (AD-39 đặt chuẩn hoá/làm sạch TRƯỚC tách Chương ⇒ trên đường tệp/dán tay chỉ Chương đầu có số đếm thật) **không** được xếp vào nhóm sạch dựa trên dấu hiệu đó — "chưa đo" phải phân biệt được với "đã đo và sạch" (nếu không, trung vị của N−1 số `0` mặc định hoá `0`). `⌥W` lọc về nhóm cần xem; `⌘↵` xác nhận toàn bộ dùng hợp âm `Mod+Alt+Enter` (né trùng lệnh xác nhận segment của Epic 2) — hợp âm cuối cùng là nợ sản phẩm, chủ Ice.
...
- **Thuật toán bóc thật đã cài đặt từ Story 6.7, không đợi tới 6.9:** AD-16 buộc kéo `Extractor`/`dom_smoothie` lên trước vì màn xem trước bắt buộc phải hiện nội dung đã bóc thật. Story 6.9 xây tiếp trên nền đó: mô hình khối phải chở cả khối **đã bị loại** kèm lý do, vì `Space`/`[`/`]` của FR123 chỉ có nghĩa khi bóc thiếu cũng sửa được — không riêng bóc thừa. `dom_smoothie::Article` chỉ phơi phần giữ lại nên không đủ; `dom_query` (duyệt lại HTML gốc bằng CSS selector) được nâng thành phụ thuộc trực tiếp của `Extractor` để dựng dãy khối cả trang kèm trạng thái giữ/loại.
...
- **Xem trước đa-Chương tách theo TẦNG, không theo mục tiêu (Story 6.10a, thêm 2026-09-08 qua correct-course):** trước story này cả bốn tầng tính từ byte của riêng Chương đầu, nên hai con số của bộ lọc "cần xem" chưa có dữ liệu để bám. 6.10a chỉ dựng đường dữ liệu theo từng Chương và con trỏ điều hướng; 6.10 giữ nguyên vế bộ lọc. `⌥` không phải phím bổ trợ chính, nên `⌥←`/`⌥→` đi theo **đúng khuôn Story 6.9 đã dựng**: vẫn là command đăng ký (AD-34) nhưng `keys: undefined`, và phím tới qua handler DOM cục bộ trên scrim lớp phủ — không phải một đường thứ hai bỏ qua `CommandRegistry`.
- **Ngưỡng "bất thường so với trung vị" dùng hàng rào Tukey — một cơ chế cho mọi tín hiệu so-tương-đối (chốt 2026-09-08):** dưới `Q1 − 1,5×IQR` hoặc trên `Q3 + 1,5×IQR`, hằng `1,5` là quy ước có tên chứ không tự đúc. N nhỏ (nhất là N=1, không có "Chương khác" để so) rơi vào "không đo được", không phải "sạch". Trường FR125 mới mang `Option<usize>` như `cleanup_match_count` (`null` = không đo được, `Some(0)` = đã đo và sạch), và **không đặt tên trần `joined_lines`** — tên đó đã thuộc `NormalizedPreviewWire` (`project.rs:1023`) với nghĩa khác (theo ứng viên bảng mã, có cửa sổ `EVIDENCE_WINDOW_BYTES`).
...
- Dưới ô dán link hiện *N link · sẽ tạo N Chương* — lệch nhau là dấu hiệu lỗi. Đầu xem trước hiện *N Chương cần xem*/*M Chương sạch*; `⌥W` lọc cần xem, `⌥←`/`⌥→` đi Chương trước/sau (dừng ở hai đầu danh sách, không đổi gì khi lớp phủ xem trước đã đóng), `⌘↵` xác nhận toàn bộ.
...
- Story 6.9, 6.6, 6.3, 6.5 nuôi trực tiếp vào 6.10a (đường dữ liệu theo từng Chương), rồi 6.10a là nền bắt buộc cho 6.10 (bộ lọc) — thứ tự thực thi 6.10a → 6.10; 6.10a còn nghiệm thu lại FR123/FR124 ở quy mô N Chương. 6.11 là nền cho 6.13, 6.14; 6.12 cấp năng lực đọc số đoạn trong ô bảng — điều kiện cho một cổng kiểm ở Epic 8.
