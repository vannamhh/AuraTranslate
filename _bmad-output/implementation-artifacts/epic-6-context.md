...
- Ba thư viện nền đã rà và ghim vào bảng Stack (Story 6.1, NFR15 — mọi thư viện mới phải rà GPL v3 trước khi thêm): `dom_smoothie` 0.18.0 (MIT) cho `Extractor`; `chardetng` 1.0.0 (Apache-2.0 OR MIT) cho dò bảng mã; `Fetcher` dùng `reqwest` 0.13.4 sẵn có (bật thêm feature `blocking`, 0 crate mới) — đã đo đạt cả ba năng lực cần (theo dõi chuyển hướng để cưỡng chế allowlist, giới hạn kích thước đọc theo dòng chảy, phân biệt lỗi kết nối/timeout). Tỉ lệ bóc/dò sai cao không chặn tiến độ, không đổi kiến trúc — sửa tay ở xem trước là dự phòng theo thiết kế. `dom_smoothie` kéo theo 10 gói bắc cầu mới, ba gói (`cssparser`, `cssparser-macros`, `selectors`) mang MPL-2.0 — đã xác nhận tương thích GPLv3 chiều đi vào, nhưng là hạng copyleft-yếu đầu tiên trong cây phụ thuộc.
- Nợ đo còn treo, liên quan trực tiếp các story sau: `chardetng` mới đo trên 0 mẫu GBK/Big5 thật (Story 6.3 cần Ice cấp fixture thật trước khi có số đo); tỉ lệ bóc sai của `dom_smoothie` mới đo trên 7 mẫu cùng một site, không suy rộng sang site khác (Story 6.9); `dom_smoothie` hiện ghim nhưng chưa được gọi (0 ký hiệu trong nhị phân release) nên dư địa NFR6 chưa tính cây con của nó — Story 6.9 (lần đầu gọi thật) phải đo lại dung lượng nhị phân. Thư viện cho đường đọc `.docx` (khác `docx-rs` đã ghim cho ghi) vẫn Deferred: cần mũi thăm dò đọc thử bảng hai cột thật trước khi Story 6.12/6.16 dựng alignment; ứng viên thay thế (`docx-reader`, `rdocx`) chưa rà giấy phép.
