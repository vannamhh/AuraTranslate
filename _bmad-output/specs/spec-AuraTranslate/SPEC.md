...
- **Thứ tự hy sinh panel khi cửa sổ hẹp là quyết định, không phải con số hiệu chỉnh được.** Đề xuất AI nhường trước; Tra cứu nhường sau nhưng **rút về thanh trạng thái, không bao giờ mất hẳn**; cặp **Nguyên văn | Bản dịch không bao giờ nhường**. Bốn ngưỡng kích hoạt (A11) sẽ được đo lại trên máy thật và đổi số (Q9) — **thứ tự này thì không đổi theo**. 🔵 *(sửa 2026-09-23, Story 4.12: bốn ngưỡng đã chốt theo phán quyết của chủ dự án, không đổi số — xem A11.)*
...
- **A6** — Ngưỡng tìm kiếm Library p95 < 500 ms trên 5.000 Chương là hợp lý. Ngưỡng tạm, đặt bằng phán đoán kỹ thuật. **Đo sơ bộ 2026-09-02** (Story 5.14, commit `a5947e4`, fixture tổng hợp 1 Work/5.000 Chương/50.000 segment, release, 3 phiên × 5 ca × 200 mẫu): p95 xấu nhất của từng ca **57,981 ms** — **dưới ngưỡng**, dư ~8,6 lần. Chưa đóng A6: fixture chưa phải thư viện thật tạo qua FR14.
- **A7** — Ngưỡng khởi động < 3 giây trên 5.000 Chương là hợp lý. Ngưỡng tạm. 🔴 **Đo sơ bộ 2026-09-02** (cùng lượt A6, 9 lần khởi động): cold median **2.581,373 ms** nhưng max **4.364,883 ms**; warm median 2.054,714 ms, max 4.083,545 ms — **vượt ngưỡng**. Đáng chú ý hơn con số: **biên độ giữa các phiên lớn hơn khoảng cách còn lại tới trần**, nên một phán quyết trên 3 phiên là mỏng. Chưa đóng A7.
- **A8** — Ngưỡng bộ nhớ nhàn rỗi < 300 MB là hợp lý. Ngưỡng tạm. 🔴 **Đo sơ bộ 2026-09-02** (cùng lượt A6, 180 mẫu idle hợp lệ, mỗi mẫu là PID app cộng WebKit mới sinh): phys_footprint lớn nhất **894.570.496 byte · 894,570 MB · 853,129 MiB** ở pha Reading fixture full — **vượt ngưỡng theo cả MB lẫn MiB**, khoảng 3 lần. Ngay pha `library` của fixture frontier đã là 305,476 MB, tức **vượt trần trước cả khi mở một Chương**. Chưa đóng A8.
...
- **A13** — Bộ dò bảng mã đoán đúng ở tỷ lệ chấp nhận được trên `.txt` GBK và Big5 lấy từ diễn đàn. `chardetng` 1.0.0 + `encoding_rs` 0.8.35 (đều tương thích GPL v3) **đã ghim 2026-09-03** (Story 6.1 — `ARCHITECTURE-SPINE.md:830`, `:1104`); **tỉ lệ dò đúng CHƯA ĐO — 0 mẫu thật**. Đường đổi bảng mã bằng tay ở màn xem trước là phương án dự phòng **theo thiết kế**, không phải vá. Cùng mũi thăm dò với A12, ở Giai đoạn 3b. 🔵 *(sửa 2026-09-15: "chưa ghim" và "Giai đoạn 3" đều đã hết đúng. Nợ có chủ Ice: `deferred-work.md:9193` — cấp `.txt` GBK/Big5 thật vào `6-1-ban-do/fixtures/encoding/` rồi chạy `webimport_probe.rs:260`.)*
- **A11** — Bốn ngưỡng bố cục màn hình hẹp là hợp lý, đo theo **vùng làm việc** (chiều cao cửa sổ trừ thanh tiêu đề và thanh trạng thái) chứ không theo kích thước màn hình: **≥ 1100×820** giữ 2×2 · **< 820 cao** gộp hàng dưới thành một panel có tab · **< 1100 rộng hoặc < 700 cao** chỉ còn Nguyên văn | Bản dịch, Tra cứu rút về ngăn kéo · **< 860 rộng** báo không hỗ trợ. Ngưỡng tạm đặt trên mockup, chưa chạy máy thật (Q9). Xem `EXPERIENCE.md`. 🔵 *(sửa 2026-09-23, Story 4.12: bốn số đã **chốt** — `1100` · `820` · `700` · `860`, giống nhau cho cả Ⓑ-1 và Ⓑ-2, bằng đúng số hạt giống. Chốt theo **phán quyết của chủ dự án**, KHÔNG phải số đo: lượt hiệu chỉnh chỉ có một phép đo (Ⓑ-2, vùng làm việc 1280×833, ép đủ ba panel — dùng được), không ngưỡng nào được đo tới điểm hỏng. Cách diễn đạt trước 2026-08-14 ở trên đã cũ, bản sống là UX-DR15 trong `epics.md`: «giữ 2×2» nay là giữ bố cục đã chọn (Ⓑ-1 hoặc Ⓑ-2) đủ ba panel · «gộp hàng dưới» nay là gộp Tra cứu và Đề xuất AI thành một panel có tab · «rút về ngăn kéo» là một mục trên thanh trạng thái mở ngăn kéo đè lên lưới. Vùng làm việc trừ **74px** (titlebar 40 + status 34, đọc token lúc chạy), không phải 38 + 32. Hệ quả đã ghi: cửa sổ mặc định 1280×860 có vùng làm việc 1280×786 nên mở ở tầng gộp tab, không phải đủ ba panel. Chi tiết: `spec-4-12-…md` §Phase 5.)*
...
- **Q4** — Hiệu chỉnh ba ngưỡng tạm A6, A7, A8. Đóng bằng đo trên thư viện thật ở **nghiệm thu cuối**. **VẪN MỞ.** Story 5.14 (2026-09-02) đã cấp số **sơ bộ** cho cả ba trên một fixture **tổng hợp** — A6 dưới ngưỡng, A7 và A8 vượt — nhưng số sơ bộ không hiệu chỉnh được ngưỡng nào: điều kiện đóng là đo trên thư viện 5.000 Chương tạo **qua sản phẩm** (FR14), và đó là **Story 10.9** 🔵 *(gộp từ Story 6.18 ngày 2026-09-15 qua `correct-course`; phép đo dời về nghiệm thu cuối)*. Không ngưỡng nào được sửa theo số của 5.14. *Chủ: chủ dự án.*
...
- **Q9** — Hiệu chỉnh bốn ngưỡng bố cục màn hình hẹp (A11). Đóng bằng **đo trên máy thật khi có bản chạy được**, từ Giai đoạn 2 ngay khi Workspace bốn panel dựng xong. Không chặn tiến độ — ngưỡng tạm vẫn nghiệm thu được. Chỉ **số** được hiệu chỉnh; thứ tự hy sinh panel là ràng buộc, không hiệu chỉnh. *Chủ: chủ dự án.* 🔵 *(ĐÃ ĐÓNG 2026-09-23, Story 4.12 — bằng **phán quyết của chủ dự án**, không bằng đo trên máy thật như điều kiện đóng ở trên nói: số hạt giống được nhận làm số chốt cho cả Ⓑ-1 và Ⓑ-2 sau một phép đo duy nhất. Xem A11.)*
