...
- Chỉ mục Library có **một đường ghi duy nhất** (`AD-8` — 🔵 **SỬA 2026-09-16 (Story 6.7b) — trích dẫn sai, sửa `AD-44` → `AD-8`**: `Indexer::rebuild` là người viết `library-index.db` DUY NHẤT theo AD-8, và `src-tauri/AGENTS.md:54` đã trích đúng từ đầu — ba tài liệu nay đồng thuận): Chương thêm vào phải đi qua đường đó. Dev kiểm lại lúc soạn spec; nếu phải đổi một bất biến thì **dừng và giao Winston**, không tự soạn `AD`.
