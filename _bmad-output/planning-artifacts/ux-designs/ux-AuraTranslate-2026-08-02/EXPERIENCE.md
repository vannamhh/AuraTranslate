...
**Đã kiểm toán 2026-08-03.** Cả **29 file** dùng token hiện hành: không màu nào trượt WCAG AA, không biến nào đã bị bỏ, không chỗ nào dùng `opacity` để làm mờ chữ, tất cả khai `Source Serif 4` / `Noto Serif CJK` / `Source Sans 3` (giữ chuỗi dự phòng phía sau để render được khi máy chưa cài font). *(Cập nhật cùng ngày: tên họ chữ Hán đổi từ `Source Han Serif` sang `Noto Serif CJK` khi Ice chốt lấy font qua kênh Google — **cùng một font, hai nhãn**, không đổi một nét chữ nào. Cả hai biến thể vùng `SC` và `TC` cùng có mặt trong chuỗi cho tới khi Story 1.1 chốt một. Xem `DESIGN.md` §Typography.)*
