...
**Gộp ngầm** — 🔵 **SỬA 2026-08-17 (Story 2.8).** Câu này mang bản UX-DR32 **CŨ** — *"gõ đè lên đúng vị trí ranh giới **là** ra lệnh gộp"*. Tiền đề đó đã bị Sprint Change Proposal 2026-08-14 bác *(trong lưới hai cột, ranh giới giữa hai câu là ranh giới **HÀNG**, không một vị trí con trỏ trong một dòng văn liền tục)*, và `epics.md` đã sửa theo: cử chỉ kích hoạt nay là **`Backspace` ở đầu ô** (Story 2.9). Ngữ nghĩa AD-5 dưới đây **không đổi một chữ**. Bản đúng: **`Backspace` ở đầu ô bản dịch** **là** ra lệnh gộp. Hệ thống thực hiện đúng ngữ nghĩa AD-5: hai câu cũ về hưu và vẫn tra lại được lịch sử, câu mới bắt đầu ở trạng thái chưa xác nhận với lịch sử rỗng. Một dòng báo ở lề. 🔵 **SỬA 2026-08-18 (Ice ký, SCP 2026-08-18b):** mệnh đề *"hoàn tác bằng `⌘Z`"* **RÚT**. Nó là **mẩu sót** của chính lượt sửa 2026-08-17 ở dòng này — lượt ấy thay cử chỉ kích hoạt nhưng để nguyên vế `⌘Z` ở cuối câu — và nó **mâu thuẫn với `:169` trong cùng tệp**: *"trên dữ liệu mà **AD-5 không cho hoàn tác**"*. Đo thêm: không FR nào đòi hoàn tác, và dòng báo đang chạy *(`vi.json:101`)* không hứa nó. **Đường quay lại là gọi lại chính lệnh gộp/tách** — cả hai là lệnh có sẵn; không byte nào bị phá, cái mất là công sức thao tác. **Không chặn, không hỏi lại** — chặn lại sẽ phá đúng cảm giác tự do mà Editor liền mạch tồn tại để có.
...
| `⌘⌥↓` | **Câu chưa dịch kế tiếp** — 🔵 **SỬA 2026-08-18 (Story 2.10, Quyết định #1(c), Ice ký):** ô này viết `⌥↓`, và phím đó **không dùng được ở ca thường nhất**. Đo trên WKWebView 605.1.15 thật: `Alt` không phải phím bổ trợ chính nên luật vùng gõ (`commands/keys.ts`) **nuốt** hợp âm khi caret đang ở trong một ô bản dịch — tức đúng lúc người dùng vừa gõ xong một câu và muốn nhảy tiếp. Đối chứng dương: cùng hợp âm với tiêu điểm **ngoài** ô thì lệnh chạy. Thêm `⌘` để nó đi qua luật ấy; `⌥↓` được trả lại cho *"xuống cuối đoạn"* của macOS. Command id **không đổi**, nên phím người dùng đã tự gán vẫn giữ nguyên. |
| `⌘⌥]` `⌘⌥[` | **Chương sau** / **Chương trước** — 🔵 **THÊM 2026-08-18 (Story 2.11, FR26).** Bảng này trước đó **không một hàng nào** cho chuyển Chương, và chỗ người ta tưởng đã đặt trước — `⌥←`/`⌥→` — hoá ra thuộc bảng *"Sửa ranh giới bóc"* ở `:184` **của màn xem trước nhập** (UX-DR33), không phải Workspace. Cặp phím trần ấy cũng **không dùng được** ở đây: `commands/keys.ts` nuốt mọi hợp âm không mang `⌘`/`Ctrl` khi caret đang trong ô bản dịch, tức đúng ca thường nhất của FR26 *(người dùng vừa gõ xong câu cuối Chương)* — cùng phép đo đã lật `⌥↓` → `⌘⌥↓` ở Story 2.10. Cặp ngoặc mang `⌘` nên nó đi qua. **Chuyển Chương flush bản dịch đang gõ TRƯỚC**, và một lượt flush trượt thì **chặn** lượt chuyển. Ở Chương đầu/cuối: báo trên thanh trạng thái, **không quay vòng**. |
| *(chưa gán)* | **Câu kế tiếp** / **Câu trước đó** — 🔵 **THÊM 2026-08-18 (Story 2.10, FR25).** Hai lệnh điều hướng **theo vị trí**: chúng **dừng** ở câu đã cắt bỏ, khác `⌘⌥↓` vốn nhảy qua *(nếu cả ba cùng nhảy qua thì không đường bàn phím nào đưa con trỏ tới một câu đã cắt bỏ để khôi phục — FR133 vế "đảo ngược được" sẽ chỉ còn đường chuột)*. **Cố ý chưa có phím mặc định** (Quyết định #2(c), Ice ký): người dùng tự gán qua màn hình phím tắt. |
...
**Đã kiểm toán 2026-08-03.** Cả **29 file** dùng token hiện hành: không màu nào trượt WCAG AA, không biến nào đã bị bỏ, không chỗ nào dùng `opacity` để làm mờ chữ, tất cả khai `Source Serif 4` / `Noto Serif CJK` / `Source Sans 3` (giữ chuỗi dự phòng phía sau để render được khi máy chưa cài font). *(Cập nhật cùng ngày: tên họ chữ Hán đổi từ `Source Han Serif` sang `Noto Serif CJK` khi Ice chốt lấy font qua kênh Google — **cùng một font, hai nhãn**, không đổi một nét chữ nào. **Cập nhật 2026-08-03 sau rà soát:** Story 1.1 đã chốt **TC**, và chuỗi `--read` của cả 29 file đã bỏ hẳn `Noto Serif CJK SC` cho khớp `DESIGN.md`. Trước lần rà này, 29 file vẫn khai `SC` **đứng trước** `TC` — tức mockup dựng trên máy có cài SC sẽ hiện đúng biến thể vừa bị bác bỏ, kể cả vị trí dấu câu, chính là điểm khác biệt mà Story 1.1 lấy làm lý do quyết định. Xem `DESIGN.md` §Typography.)*
...
- ~~**Font thật chưa đo**~~ — ✅ **đóng 2026-08-03** (Story 1.1, mũi thăm dò kỹ thuật). Cả ba câu hỏi đã có số: **dung lượng** — chênh lệch `.dmg` do font là **20,300 MiB = 21,29 MB**, tổng với database 130 MB hiện tại là **151,29 MB**, dưới trần `NFR6`; **giấy phép** — **SIL OFL 1.1** cả ba, tương thích `GPL v3` theo diện gộp gói, đã ghi ba hàng vào bảng Stack; **biến thể vùng** — chốt **`Noto Serif CJK TC`**. Hai việc mang theo, **không chặn thiết kế**, cả hai nay đã thành AC thật: `.msi` chưa đo được trên macOS (Story 1.3), và dư địa dưới trần chỉ còn **~47 MB** cho các nguồn từ điển còn lại **cộng toàn bộ mã sản phẩm chưa viết** (Story 1.9) — cả hai thuộc tầng PRD/kỹ thuật, không đổi một quyết định thiết kế nào. 🔄 **CẬP NHẬT 2026-08-05:** con số *dư địa ~47 MB* là bản ghi tại thời điểm, không còn sống. NFR6 sửa lần hai — trần **400.000.000 byte**, payload thật **343.991.430 byte** với bảy nguồn (ĐẠT, dư 56.008.570). Vẫn không đổi một quyết định thiết kế nào. Số đo và ảnh chụp: [`research/font-spike-results-2026-08-03.md`](../../research/font-spike-results-2026-08-03.md).

  **Một điều bất ngờ đáng ghi cho tầng thiết kế.** Bàn giao cho mũi thăm dò nói khác biệt TC/SC là *"dáng chữ ưu tiên ở mã dùng chung"* — đúng, nhưng **không phải phần quan trọng nhất**. Bản dựng thật cho thấy khác biệt lớn hơn nằm ở **vị trí dấu câu**: TC đặt 「，」 và 「。」 giữa ô chữ, SC đặt chúng ở góc dưới bên trái. Dáng chữ chỉ khác ở một nhúm mã; dấu câu khác ở **mọi câu**. Bài học: khi so hai biến thể font, đừng chỉ so bảng ký tự — dựng một **đoạn văn thật** rồi nhìn.
