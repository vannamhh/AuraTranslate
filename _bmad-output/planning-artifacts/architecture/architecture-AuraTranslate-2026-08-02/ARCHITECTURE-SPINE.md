...
⚠️ **`dom_smoothie` kéo theo một cây con 12 gói MỚI, ba trong số đó KHÔNG dễ dãi.** Đo bằng `cargo metadata` trước/sau (593 → 605 gói): `cssparser` 0.37.0, `cssparser-macros` 0.7.0, `dom_query` 0.28.0, `flagset` 0.4.7, `gjson` 0.8.1, `html-escape` 0.2.15, `html5ever` 0.39.0, `markup5ever` 0.39.0, `nom` 8.0.0, `selectors` 0.38.0 — cộng `chardetng` chính nó. Ba gói `cssparser` · `cssparser-macros` · `selectors` khai **MPL-2.0** (Mozilla Public License), không nằm trong nhóm dễ dãi thường lệ của bảng này (MIT · Apache-2.0 · BSD-3-Clause · BSD-2-Clause · ISC · OFL 1.1). MPL-2.0 **vẫn tương thích GPLv3 chiều đi vào** — điều khoản 3.3 của chính văn bản MPL 2.0 cho phép kết hợp mã MPL vào một "Larger Work" phát hành dưới GPL/LGPL/AGPL, và FSF liệt MPL-2.0 vào danh sách *"GPL-compatible"* trên trang giấy phép của họ — nhưng đây là hạng giấy phép copyleft-yếu đầu tiên lọt vào cây, khác hẳn mọi crate khác trong bảng Stack. Chín gói còn lại (`dom_query` · `flagset` · `gjson` · `html-escape` · `html5ever` · `markup5ever` · `nom`) đều MIT hoặc Apache-2.0/MIT, không hàng mới. 🔵 **Đo bổ sung 2026-09-03 — mệnh đề "MPL-2.0 tương thích GPLv3" ở trên vốn được KHẲNG ĐỊNH, nay được ĐO.** Điều 3.3 chỉ áp khi tệp KHÔNG mang thông báo Exhibit B (*"Incompatible With Secondary Licenses"*) — một tệp mang Exhibit B thì kết hợp vào GPL bị chặn, và khác biệt ấy nằm ở HEADER TỪNG TỆP chứ không ở trường `license`. Đếm trên nguồn đã tải: `cssparser` 0.37.0 **0** tệp `.rs`, `cssparser-macros` 0.7.0 **0**, `selectors` 0.38.0 **0** — cả ba mang Exhibit A thường lệ (*"This Source Code Form is subject to the terms of the Mozilla Public License, v. 2.0"*). ⇒ Kết luận đứng, và nay kiểm được. ⚠️ Ba lần "hit" khi `grep` chuỗi đó trong `LICENSE` là **văn bản mẫu** của chính MPL 2.0 (`LICENSE:28`, `:353`, `:369`), không phải một thông báo áp lên mã — đừng đọc nhầm chúng thành cờ đỏ. ⚠️ **`selectors` 0.38.0 KHÔNG ship tệp giấy phép nào** (`ls` trong nguồn đã tải: không `LICENSE*`); bằng chứng giấy phép duy nhất của nó là header từng tệp. NFR15 điều 1 nói "mở tệp giấy phép mà đọc" — với gói này, thứ mở được là header, và điều đó phải ghi ra thay vì để người sau tưởng đã có một tệp được đọc. Theo đúng AC "bảng Stack có 37 hàng (34+3)", chín gói bắc cầu này **không** có hàng riêng — ba tên gốc (`dom_smoothie`/`chardetng`/`encoding_rs`) là thứ Story 6.1 khai tường minh; ghi ra ở đây để không ai đọc bảng Stack thành "cây phụ thuộc mới chỉ có ba crate".
